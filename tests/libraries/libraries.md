...
| 111 | AI SDK OpenAI Provider | `@ai-sdk/openai` | ⚠️ | 2026-03-17 | 4/4 offline + 1/1 live tests pass in Node.js; in wasm-rquickjs 2/4 offline pass (factory/tools), while mocked chat/responses request paths fail with `cannot read property 'Symbol.iterator' of undefined` in `__wasm_rquickjs_builtin/http`; live chat completion passes when `OPENAI_API_KEY` is forwarded to wasmtime |
