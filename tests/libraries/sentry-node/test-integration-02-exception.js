...
export const run = async () => {
  ...
  Sentry.captureException(new Error('integration boom'));
  ...
  assert.strictEqual(state.lastExceptionValue, 'integration boom');

  return 'PASS: captureException sends exception payload to the mock HTTP endpoint';
};
