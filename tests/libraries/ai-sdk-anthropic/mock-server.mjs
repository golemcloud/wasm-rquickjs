...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && path === '/v1/messages') {
    ...
    if (promptText === 'TRIGGER_ANTHROPIC_ERROR') {
    ...
    if (promptText === 'CHECK_HEADERS') {
      ...
      const hasCustomHeader = req.headers['x-extra-header'] === 'present';

      if (!hasApiKey || !hasVersion || !hasCustomHeader) {
    ...
    return sendJson(res, 200, {
      id: 'msg_mock_generate',
      type: 'message',
      role: 'assistant',
      model: requestBody?.model ?? 'claude-3-haiku-20240307',
      content: [{ type: 'text', text: `MOCK_OK:${promptText}` }],
      stop_reason: 'end_turn',
      stop_sequence: null,
      usage: {
        input_tokens: 4,
