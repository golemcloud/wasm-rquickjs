...
- **wasm-rquickjs:** N/A (skipped — bundle fails on Node.js)
- **Error:** `createCommonjsRequire(...).ensure is not a function`
- **Root cause:** The bundled `pdf.js` runtime path uses `require.ensure` through Rollup's generated `createCommonjsRequire(...)`, which is not supported.
...
- **Impact:** Functional `pdf-parse` APIs cannot be validated in Node.js bundled mode, so wasm-rquickjs execution is skipped for tests 01–04.
...
- Behavioral differences: Not reached due to bundle initialization failure
