...
export const run = async () => {
  ...
  app.use(async (ctx, next) => {
    ctx.state.order = ['a'];
    await next();
    ctx.state.order.push('c');
    ctx.set('x-order', ctx.state.order.join(''));
  });

  app.use(async (ctx, next) => {
    ctx.state.order.push('b');
    await next();
  });

  app.use(async (ctx) => {
    ctx.assert(ctx.state.order.join('') === 'ab', 500, 'unexpected order');
  ...
  assert.strictEqual(res.body, 'middleware-ok');
  assert.strictEqual(res.getHeader('x-order'), 'abc');

  return 'PASS: middleware onion order and ctx.state propagation work';
};
