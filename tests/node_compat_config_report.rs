...
fn is_accepted_module_known_gap_reason(reason: Option<&str>) -> bool {
    const ACCEPTED_REASONS: &[&str] = &[
        ...
        "rooted symlink targets cannot be resolved inside a WASI preopen; the adapter rejects them at creation, while persistent relative symlinks are covered by runtime tests",
