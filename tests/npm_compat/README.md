...
| `npm exec` | Constrained | Local and deterministic-registry JavaScript bins execute through persistent `.bin` symlinks in fresh runtimes. Native binaries, arbitrary shells, and host executables remain unsupported. |
...
| Links | `--bin-links`, `--install-links` | Constrained | Persistent `.bin`, workspace, `npm link`, and representative pnpm-style relative links preserve readlink, realpath, and module identity. Rooted link targets are rejected by WASI. |
