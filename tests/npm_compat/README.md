...
| Network and cache | `--registry`, `--offline`, `--prefer-offline`, retry and proxy settings | Constrained | Registry metadata/tarballs go through `wasi:http`; Agent-provided socket hooks are ignored with a warning rather than bypassing the component transport. Fresh `npm ci --offline` restores from cache. A hanging request times out and releases execution capacity. Prefer-offline, proxies, and redacted failures remain planned. |
