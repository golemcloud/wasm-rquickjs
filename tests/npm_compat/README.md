...
| TypeScript entry points | Planned | Runtime TypeScript support exists separately; installation and execution as an npm package class are not covered here. |
...
lifecycle/run scripts, tree inspection and mutation, init, pack dry-run, and the
actual npm exec/npx failure boundary. Separate component instances also perform
different installs concurrently, and one then repeats a clean install without
workspace state crossing between them.
