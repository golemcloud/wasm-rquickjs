...
use wit_parser::{Docs, Function, FunctionKind, Handle, Type, TypeDefKind, TypeId};
...
pub enum RustType {
    ...
    /// `wit_bindgen_p3::rt::async_support::FutureReader<T>` (WASI Preview 3 `future<T>`).
    /// `payload` is `None` for the untyped `future` (rendered as `FutureReader<()>`).
    Future {
        payload: Option<Box<RustType>>,
    },
    /// `wit_bindgen_p3::rt::async_support::StreamReader<T>` (WASI Preview 3 `stream<T>`).
    /// `payload` is `None` for the untyped `stream` (rendered as `StreamReader<()>`).
    Stream {
        payload: Option<Box<RustType>>,
    },
    Unit,
...
impl RustType {
    ...
    fn from_anonymous_type(context: &GeneratorContext<'_>, id: TypeId, mode: TypeMode) -> Self {
        ...
        match &ty.kind {
            ...
            TypeDefKind::Future(inner) => {
                let payload = inner.as_ref().map(|t| {
                    let inner_mode = type_mode_for(context, t, TypeOwnershipStyle::Owned, "'_");
                    Box::new(Self::from_type(context, t, inner_mode))
                });
                Self::Future { payload }
            }
    ...
    pub fn adjust_export_side(self) -> RustType {
        match self {
            ...
            RustType::Future { payload } => RustType::Future {
                payload: payload.map(|p| Box::new(p.adjust_export_side())),
            },
            RustType::Stream { payload } => RustType::Stream {
                payload: payload.map(|p| Box::new(p.adjust_export_side())),
            },
            _ => self,
        }
    ...
    pub fn contains(&self, condition: impl Fn(&RustType) -> bool + Clone) -> bool {
        if condition(self) {
            ...
            match self {
                ...
                RustType::Future { payload } | RustType::Stream { payload } => payload
                    .as_ref()
                    .map(|p| p.contains(condition))
                    .unwrap_or(false),
                _ => false,
            }
        }
    ...
    pub fn in_iter(&self) -> RustType {
        ...
        match self {
            ...
            RustType::Future { .. } => self.clone(),
            RustType::Stream { .. } => self.clone(),
...
impl Display for RustType {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            RustType::Future { payload } => match payload {
                Some(p) => write!(f, "wit_bindgen_p3::rt::async_support::FutureReader<{}>", p),
                None => write!(f, "wit_bindgen_p3::rt::async_support::FutureReader<()>"),
            },
            RustType::Stream { payload } => match payload {
                Some(p) => write!(f, "wit_bindgen_p3::rt::async_support::StreamReader<{}>", p),
                None => write!(f, "wit_bindgen_p3::rt::async_support::StreamReader<()>"),
            },
...
impl RustFunctionParameter {
    pub fn for_guest_import(
        ...
        // Synchronous imports follow wit-bindgen's borrowed parameter convention
        // (`&str`, `&[T]`, ...). Async imports (Preview 3) are generated by wit-bindgen
        // with *owned* parameters (`String`, `Vec<T>`, ...) because the lowered call may
        // outlive the caller's borrow.
        let style = if is_async {
            TypeOwnershipStyle::Owned
        } else {
    ...
    pub fn new(context: &GeneratorContext<'_>, name: &str, function: &Function) -> Self {
        ...
        Self {
            ...
            import_parameters: function
                ...
                .iter()
                .map(|param| {
                    RustFunctionParameter::for_guest_import(
                        context,
                        &param.name,
                        &param.ty,
                        is_async_import,
