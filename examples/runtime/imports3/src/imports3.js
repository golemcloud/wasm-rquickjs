...
export const test = async () => {
    ...
    console.log(`f1: ${await types.f1([0.1, 0.2, 0.3], ["a", "b"], ["c", "d"])}`);
    // f2: func(a: option<string>) -> option<u32>;
    console.log(`f2: ${await types.f2("hello world")}`);
    // f3: func(a: bool, b: s8, c: s16, d: s32, e: s64, f: u8, g: u16, h: u32, i: u64, j: f32, k: f64, l: char, m: string)
    console.log(`f3: ${await types.f3(true, -8, -16, -32, -64n, 8, 16, 32, 64n, 3.14, 2.718281828459045, 'c', "hello world")}`);
    // f4: func(a: result<s32, string>) -> result<s32, string>;
    console.log(`f4: ${JSON.stringify(await as_result(() => types.f4({tag: 'ok', val: 42})))}`);
    // f5: func(a: result<_, string>) -> result<_, string>;
    console.log(`f5: ${JSON.stringify(await as_result(() => types.f5({tag: 'err', val: 'error message'})))}`);
    // f6: func(a: result<string>) -> result<string>;
    console.log(`f6: ${JSON.stringify(await as_result(() => types.f6({tag: 'ok', val: 'success'})))}`);
    // f7: func(a: result) -> result;
    console.log(`f7: ${JSON.stringify(await as_result(() => types.f7({tag: 'ok'})))}`);
    // f8: func(a: tuple<string, u32, f32>);
    console.log(`f8: ${await types.f8(['example', 123, 4.56])}`);
    ...
    //     blue,
    // }
    const f11 = await types.f11('blue');
    console.log(`f11: ${f11}`);
    // f12: func(a: string) -> option<color>;
    const f12 = await types.f12('green');
    ...
    //   execute,
    // }
    const f13 = await types.f13({read: true, write: false, execute: true});
    console.log(`f13: ${JSON.stringify(f13)}`);
    // f14: func(a: u64) -> u64;
    const f14 = await types.f14(1234567890n);
    console.log(`f14: ${f14}`);
    // f15: func(bytes: list<u8>) -> list<u8>
    const f15 = await types.f15(new Uint8Array([1, 2, 3, 4, 5]));
    console.log(`f15: ${f15}`);
